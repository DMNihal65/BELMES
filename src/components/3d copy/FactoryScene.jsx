...
const TurningMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  ...
  const statusHeight = 1.2; // Adjust based on actual model height
...
const MillingMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  ...
  // Calculate appropriate status indicator height for the larger scale
  const statusHeight = 1.0; // Adjust based on actual model height
...
const EDMMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  ...
  // Calculate appropriate status indicator height for the larger scale
  const statusHeight = 1.5; // Adjust based on actual model height
    ...
    switch(type) {
      case 'turning':
        ...
        return [
          ...
          -15 + index * 6 // Z position (spaced evenly in a row)
        ];
      case 'milling':
        ...
        return [
          ...
          -12 + index * 7 // Z position (spaced evenly in a row)
        ];
      case 'edm':
        // In the EDM room at the far end
        return [
          -5 + index * 10, // X position (centered, spaced apart)
          4,               // Y position (floor level)
    ...
    <div className={`relative h-full w-full ${className}`}>
      ...
      >
        <Suspense fallback={null}>
          ...
          <fog attach="fog" args={['#d4d4d8', 50, 120]} />
          <Sky 
            distance={450000} 
            sunPosition={[10, 5, 10]} 
            inclination={0.5} 
        ...
        {[
          ...
          <React.Fragment key={`ceiling-light-${index}`}>
            ...
            <mesh position={[position[0], position[1] + 0.5, position[2]]}>
              ...
              <meshStandardMaterial color="#f8fafc" emissive="#fff6e0" emissiveIntensity={0.2} />
            </mesh>
          </React.Fragment>
        ))}
      </group>
...
// TV component
const LargeScreenTV = ({ position, rotation = [0, 0, 0], size = [10, 6], content = "Status" }) => {
...
const Staircase = ({ position = [0, 0, 0] }) => {
  return (
    <group position={position}>
      ...
      <mesh position={[0, 5.5, -12]} castShadow receiveShadow>
