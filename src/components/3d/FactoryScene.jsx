...
const TurningMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  const gltf = useLoader(GLTFLoader, `${import.meta.env.BASE_URL}turning.glb`);
  const model = useRef();
...
const MillingMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  const gltf = useLoader(GLTFLoader, `${import.meta.env.BASE_URL}machine.glb`);
  const model = useRef();
  const statusLightRef = useRef();
  const [hovered, setHovered] = useState(false);
...
const EDMMachineModel = ({ position, rotation, scale = 0.5, onClick, isSelected, machineData }) => {
  const gltf = useLoader(GLTFLoader, `${import.meta.env.BASE_URL}wireedm.glb`);
  const model = useRef();
  const statusLightRef = useRef();
  const [hovered, setHovered] = useState(false);
        ...
        <WebGLErrorBoundary onError={handleCreationError}>
          ...
          >
            ...
            <Suspense fallback={null}>
              ...
              {quality !== 'high' && (
                <Environment
                  files={`${import.meta.env.BASE_URL}warehouse.hdr`}
                  background={false}
                  blur={0.7}
                />
              )}
    ...
    <group position={position}>
      ...
      <group>
        ...
        <group position={[0, 0, 5.05]}>
          ...
          <Text
            position={[0, 11, 0]}
            font={`${import.meta.env.BASE_URL}fonts/Rubik-VariableFont_wght.ttf`}
            fontSize={1.5}
            color="#1e3a8a"
            anchorX="center"
            anchorY="middle"
