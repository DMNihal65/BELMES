...
const JobOperationsTable = ({ jobId, onOperationEdit, operations: initialOperations, partNumber, productionOrder ,orderNumber}) => {
  ...
  const [ipidCounts, setIpidCounts] = useState({});
  ...
  const handleIpidUpload = async (file) => {
    try {
      ...
      // Add delay before refreshing counts to ensure server has processed the upload
      setTimeout(async () => {
        await fetchIpidCounts();
      }, 1000);
    } catch (error) {
  ...
  // Add this function at the top of your component to get the token
  const getAuthToken = () => {
    return localStorage.getItem('token'); // Assuming token is stored with key 'token'
  };

  // Update the fetchMppDocuments function
  const fetchMppDocuments = async (productionOrder) => {
    try {
      const token = getAuthToken();
      const response = await fetch(
        `http://172.18.7.85:4413/api/v1/documents/mpp/${productionOrder}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        console.error('Failed to fetch MPP documents:', response.statusText);
        return null;
      }
    } catch (error) {
  ...
  const handleMppView = async (operation) => {
    try {
      // First fetch the MPP documents for the production order
      const docs = await fetchMppDocuments(orderNumber);
      
      if (!docs || docs.length === 0) {
        // If no documents exist, open manual entry form
        onOperationEdit({
          ...operation,
      ...
      const activeDoc = docs.find(doc => doc.is_active);
      ...
      const downloadResponse = await fetch(
        `http://172.18.7.85:4413/api/v1/documents/${activeDoc.id}/download`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'accept': 'application/json'
      ...
      if (downloadResponse.ok) {
        ...
        message.error('Failed to download MPP document');
        onOperationEdit({
          ...operation,
          operation_number: operation.operation_number,
          partNumber: partNumber
      ...
      message.error('Failed to process MPP document');
      onOperationEdit({
        ...operation,
        operation_number: operation.operation_number,
        partNumber: partNumber
  ...
  // Update the useEffect to fetch documents when orderNumber changes
  useEffect(() => {
    if (orderNumber) {
  ...
  // Add this new function to fetch IPID counts
  const fetchIpidCounts = async () => {
    try {
      const token = getAuthToken();
      console.log('Fetching IPID counts for order:', orderNumber);
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/api/v1/documents/ipid/${orderNumber}/counts`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'accept': 'application/json'
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Received IPID counts:', data);
        setIpidCounts(data);
      } else {
        console.error('Failed to fetch IPID counts:', response.status);
      }
    } catch (error) {
      console.error('Error fetching IPID counts:', error);
    }
  };

  // Make sure useEffect is properly set up
  useEffect(() => {
    if (orderNumber) {
      console.log('OrderNumber changed, fetching IPID counts');
      fetchIpidCounts();
    }
  }, [orderNumber]);
  ...
  const columns = [
    ...
    {
      title: 'IPID Status',
      dataIndex: 'ipid_status',
      width: 120,
      render: (_, record) => {
        const count = ipidCounts[record.operation_number] || 0;
        console.log('Rendering IPID status for operation:', record.operation_number, 'count:', count);
        return (
          <Tag color={count > 0 ? '#52c41a' : 'default'} style={{ minWidth: '80px', textAlign: 'center' }}>
            {count > 0 ? `${count} Uploaded` : 'No Files'}
          </Tag>
        );
      }
    },
    {
      ...
      render: (_, record) => {
        const editable = isEditing(record);
        const ipidCount = ipidCounts[record.operation_number] || 0;
        ...
        return (
          <Space>
            ...
            {/* IPID Upload button with more prominent color change */}
            <Tooltip title={`Upload IPID File (${ipidCount} uploaded)`}>
              <Button 
                type={ipidCount > 0 ? "primary" : "link"}
                icon={<UploadOutlined />}
                style={{ 
                  backgroundColor: ipidCount > 0 ? '#52c41a' : undefined,
                  borderColor: ipidCount > 0 ? '#52c41a' : undefined,
                  color: ipidCount > 0 ? 'white' : undefined
                }}
                onClick={() => {
