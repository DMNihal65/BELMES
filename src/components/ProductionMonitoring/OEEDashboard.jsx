...
const OEEDashboard = () => {
  ...
  const fetchOverallOEEAnalytics = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:7708/production_monitoring/overall-oee-analytics/?${params.toString()}`
      );
  ...
  const loadShiftSummaryData = async () => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:7708/production_monitoring/detailed-shift-summary/?${params.toString()}`
      );
  ...
  const fetchAllMachinesOEE = async () => {
    ...
    try {
      ...
      const promises = machineIds.map(id => 
        axios.get(`http://172.18.7.85:7708/production_monitoring/machine-oee-analysis/${id}?start_date=${formattedStartDate}&end_date=${formattedEndDate}`)
      );
  ...
  const showTrendModal = async (machineId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:7708/production_monitoring/machine-oee-analysis/${machineId}?start_date=${formattedStartDate}&end_date=${formattedEndDate}`
      );
