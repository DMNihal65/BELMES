...
const OrderTracking = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const res = await axios.get('http://172.16.0.203:8002/api/v1/auth/api/v1/auth/users-get?active_only=true');
        setUsers(res.data);
      } catch (err) {
  ...
  const handleOrderSelect = async (value) => {
    ...
    try {
      ...
      const { data } = await axios.get(`http://172.16.0.203:8002/api/v1/operatorlogs2/production-order-operations-status/${productionOrder}`);
  ...
  const handleUpdateSubmit = async () => {
    try {
      ...
      await axios.post('http://172.16.0.203:8002/api/v1/operatorlogs2/operator-log', payload);
  ...
  const fetchReportData = async () => {
    ...
    try {
      const [orderResponse, operatorsResponse, machinesResponse] = await Promise.all([
        axios.get(`http://172.16.0.203:8002/api/v1/operatorlogs2/production-order-report/${selectedOrder.production_order}`),
        axios.get(`http://172.16.0.203:8002/api/v1/operatorlogs2/production-order-report-operators/${selectedOrder.production_order}`),
        axios.get(`http://172.16.0.203:8002/api/v1/operatorlogs2/production-order-report-machines/${selectedOrder.production_order}`)
      ]);
      setReportData({
        ...orderResponse.data,
  ...
  const fetchDailyReportData = async () => {
    ...
    try {
      const response = await axios.get(`http://172.16.0.203:8002/api/v1/operatorlogs2/production-order-daily-report/${selectedOrder.production_order}`);
  ...
  const columns = [
    ...
    { 
      ...
      render: (_, record) => {
        const acceptedQty = (record.completed_quantity || 0) - (record.rejected_quantity || 0);
        const requiredQty = record.completed_quantity || 0;
