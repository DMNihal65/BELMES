...
const ProductionSchedule = () => {
  ...
  const handleDateChange = (dates) => {
    console.log('ProductionSchedule handleDateChange:', {
      receivedDates: dates?.map(d => d?.format('YYYY-MM-DD HH:mm:ss'))
    });
    
    if (!dates) {
      setGanttDateRange([dayjs().startOf('day'), dayjs().endOf('day')]);
      return;
    }
  
    // Use the dates directly without modification
    setGanttDateRange(dates);
  };
