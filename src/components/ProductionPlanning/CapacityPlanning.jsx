...
const CapacityPlanning = () => {
  ...
  const [oneShiftDays, setOneShiftDays] = useState([]);
...
const handleSaveDateConfig = () => {
  ...
  setDateConfigs(prev => {
    ...
    const filtered = prev.filter(item => item.date !== newConfig.date);
    return [...filtered, newConfig];
  });

  message.success(`Saved config for ${selectedDate.format('DD-MM-YYYY')}`);
...
const handleHolidaySelect = (dates) => setHolidays(dates);
const handleOneShiftSelect = (dates) => setOneShiftDays(dates);
