...
const JobOperationsTable = ({ jobId, onOperationEdit, operations: initialOperations, partNumber, productionOrder, orderNumber }) => {
  ...
  const save = async (key) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:1717/api/v1/planning/operations/${partNumber}/${record.operation_number}?production_order=${updateData.production_order}`, 
        {
          method: 'PUT',
          headers: {
  ...
  const fetchIpidDocumentDetails = async (operationNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:1717/api/v1/document-management/ipid/details?production_order=${currentProductionOrder}&operation_number=${operationNumber}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleVersionUpload = async (values) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:1717/api/v1/document-management/ipid/${existingIpidDocument.id}/versions`,
        {
          method: 'POST',
          headers: {
  ...
  const handleMachineSave = async (machineId) => {
    try {
      ...
      // Use the correct URL format: /api/v1/planning/operations/{part_number}/{operation_number}
      const response = await fetch(
        `http://172.18.7.88:1717/api/v1/planning/operations/${partNumber}/${selectedOperationForMachine.operation_number}?production_order=${currentData.production_order}`, 
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
