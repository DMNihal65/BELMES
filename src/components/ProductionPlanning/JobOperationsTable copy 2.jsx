...
const JobOperationsTable = ({ jobId, onOperationEdit, operations: initialOperations, partNumber, productionOrder, orderNumber }) => {
  ...
  const fetchIpidDocumentDetails = async (operationNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4494/api/v1/document-management/ipid/details?production_order=${currentProductionOrder}&operation_number=${operationNumber}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  const handleVersionUpload = async (values) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4494/api/v1/document-management/ipid/${existingIpidDocument.id}/versions`,
        {
          method: 'POST',
          headers: {
