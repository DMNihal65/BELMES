...
const JobOperationsTable = ({ jobId, onOperationEdit, operations: initialOperations, partNumber, productionOrder, orderNumber, status = 'Inactive' }) => {
  ...
  const save = async (key) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4455/api/v1/planning/operations/${partNumber}/${record.operation_number}?production_order=${updateData.production_order}`, 
        {
          method: 'PUT',
          headers: {
  ...
  const handleMachineSave = async (machineId) => {
    try {
      ...
      // Use the correct URL format: /api/v1/planning/operations/{part_number}/{operation_number}
      const response = await fetch(
        `http://172.18.7.88:4455/api/v1/planning/operations/${partNumber}/${selectedOperationForMachine.operation_number}?production_order=${currentData.production_order}`, 
        {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${token}`,
