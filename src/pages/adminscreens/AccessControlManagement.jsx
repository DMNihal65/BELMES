...
const AccessControlManagement = ({onSuccess }) => {
  ...
  const { registerUser } = useAccessControlStore();
    ...
    const fetchTotalUsersCount = async () => {
      try {
        const response = await axios.get('http://172.18.7.88:4463/api/v1/auth/users-count');
  ...
  const fetchMachineCredentials = async () => {
    try {
      const response = await axios.get('http://172.18.7.88:4463/api/v1/auth/get-machine-credentials');
  ...
  const handleUpdateMachinePassword = async (values) => {
    try {
      await axios.put(`http://172.18.7.88:4463/api/v1/auth/machine-credentials/${selectedMachine.machine_id}`, {
  ...
  const handleDeleteMachineCredential = (machineId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`http://172.18.7.88:4463/api/v1/auth/machine-credentials/${machineId}`);
  ...
  const handleDelete = (userId) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          await axios.delete(`http://172.18.7.88:4463/api/v1/auth/users/${userId}`);
  ...
  const fetchRolesData = async () => {
    try {
      const response = await axios.get('http://172.18.7.88:4463/api/v1/auth/roles');
  ...
  const handleUpdateRole = async (values) => {
    try {
      await axios.put(`http://172.18.7.88:4463/api/v1/auth/roles/${selectedRole.id}`, {
