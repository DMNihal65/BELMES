  ...
  const fetchMachines = async () => {
    ...
    try {
      const response = await axios.get(
        "http://172.18.7.88:6643/api/v1/master-order/all-machines/"
      );
  ...
  const onFinish = async (values) => {
    ...
    try {
      const response = await axios.post(
        `http://172.18.7.88:6643/api/v1/auth/register-machine-password?machine_id=${machineName}&password=${password}`
      );
