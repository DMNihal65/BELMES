...
const InventoryAllData = () => {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const response = await axios({
        url: 'http://172.18.7.85:6639/api/v1/api/inventory/items/bulk/',
        method: 'POST',
        responseType: 'blob',
        headers: {
  ...
  const handleDownloadTemplate = () => {
    window.open('http://172.18.7.85:6639/api/v1/api/inventory/items/bulk/', '_blank');
  };
  ...
  const handleExcelUpload = (file) => {
    ...
    axios.post('http://172.18.7.85:6639/api/v1/api/inventory/items/bulk/', formData, {
      headers: {
