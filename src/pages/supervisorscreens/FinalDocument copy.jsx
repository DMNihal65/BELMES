...
const VersionManagementModal = ({ visible, document, onClose }) => {
  ...
  const { fetchDocumentVersions, uploadNewVersion, updateVersion, deleteVersion, fetchFolderDocuments } = useDocumentStore();
...
const DocumentManagement = () => {
  ...
  const handleFolderSelect = async (selectedKeys, info) => {
    const clickedOutsideTree = !selectedKeys.length;
    
    if (clickedOutsideTree) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <Modal
        title={`Preview - ${selectedDocument?.name || ''} ${selectedDocument?.version_number ? `(v${selectedDocument.version_number})` : ''}`}
