...
import { Table, Switch, Card, Button, Form, Space, Row, Col, DatePicker, Tag, Input, Dropdown, Menu, Typography, Statistic, Tabs, Calendar, Radio, message } from 'antd';
...
export default function MachineMaintenance() {
  ...
  // New state for Working Hours
  const [workingHours, setWorkingHours] = useState({}); // { [date]: { isWorking: boolean, shifts: number } }
  const [selectedDate, setSelectedDate] = useState(null);
  const [workingForm] = Form.useForm();
  const calendarRef = useRef(null); // Ref to get calendar height
  const [calendarHeight, setCalendarHeight] = useState(0);

    // Replace your existing workingHours state with this
  const [shiftData, setShiftData] = useState({}); // { "2025-01-05": { id: 5, working_day: true, no_of_shifts: 2 } }
  const [loadingShift, setLoadingShift] = useState(false);

  // Fetch all shift hours on mount
  useEffect(() => {
    fetchShiftHours();
  }, []);
    ...
    try {
      ...
      data.forEach(item => {
        mapped[item.date] = {
          ...
          no_of_shifts: item.no_of_shifts
        };
      });
      setShiftData(mapped);
    } catch (err) {
      console.error(err);
      toast.error('Failed to load working hours data');
    } finally {
      setLoadingShift(false);
    }
  };

  // Save or Update shift hour
  const saveShiftHour = async () => {
    if (!selectedDate) return;

    const formValues = workingForm.getFieldsValue();
    const isWorking = formValues.isWorking;
    const shifts = isWorking ? formValues.shifts : 0;

    const existing = shiftData[selectedDate];

    try {
      const payload = {
        working_day: isWorking,
        no_of_shifts: shifts
      };
      ...
      if (existing?.id) {
        ...
        // Create new (your backend should support POST to same endpoint without ID)
        const res = await fetch('http://127.0.0.0.1:8000/api/v1/planning/shift-hours', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      toast.success(`Saved ${selectedDate} successfully!`);
    } catch (err) {
      console.error(err);
  ...
  useEffect(() => {
    // Update calendar height on mount and window resize
    const updateHeight = () => {
  ...
  const save = async (record) => {
    try {
      ...
      await updateMachineStatus(record.machine_id, formattedData);
  ...
  // Handle date selection for Working Hours
  const handleDateSelect = (date) => {
    const formattedDate = date.format('YYYY-MM-DD');

    // Block Sundays
    if (date.day() === 0) {
      message.error('Sundays cannot be selected.');
      return;
    }

    setSelectedDate(formattedDate);

    const data = shiftData[formattedDate] || { working_day: true, no_of_shifts: 1 };

    workingForm.setFieldsValue({
      isWorking: data.working_day,
      shifts: data.working_day ? data.no_of_shifts : null
    });
  };
  // Handle saving working hours configuration
  const handleWorkingHoursSave = async () => {
    ...
    try {
      await workingForm.validateFields();
      await saveShiftHour();

    } catch (err) {
      message.error('Validation failed or save error');
    }
  };
  ...
  // Render date cell with working hours info
  const dateCellRender = (value) => {
    const formattedDate = value.format('YYYY-MM-DD');
    const data = shiftData[formattedDate];

    if (!data) return null; // No config yet → empty

    return (
      <div className="text-center">
        {data.working_day ? (
          <Tag color="green" className="m-0">
            {data.no_of_shifts} Shift{data.no_of_shifts > 1 ? 's' : ''}
          </Tag>
        ) : (
          <Tag color="red" className="m-0">Non-Working</Tag>
        )}
      </div>
    );
  };
  ...
  const filteredData = machines
    .filter(machine => machine.machine_make !== 'Default')
    .filter(machine => {
  ...
  const AvailabilityContent = (
    <div>
      ...
      <div
        className="bg-white p-4 md:p-6 rounded-2xl shadow-lg border border-indigo-600"
        style={{
  ...
  // New Working Hours Content with Sidebar
    ...
    <div style={{ display: "flex", gap: "20px", flexWrap: "wrap" }}>
      ...
      >
        <Title level={5} className="mb-6">
          {selectedDate ? `Configure: ${dayjs(selectedDate).format('DD MMM YYYY')}` : "Select a date to configure"}
        </Title>

        <Form form={workingForm} layout="vertical">
          <Form.Item
            name="isWorking"
            label="Day Type"
            rules={[{ required: true }]}
    ...
    <div className="w-full bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 bg-fixed">
      ...
      <Tabs
        ...
        items={[
          ...
          { key: 'working-hours', label: 'Shift Hours Configuration', children: WorkingHoursContent }
        ]}
      />
    </div>
  );
