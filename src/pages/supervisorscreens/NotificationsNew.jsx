  ...
  const handleAcknowledge = async (notification) => {
    ...
    try {
      ...
      if (notification.notificationType === 'material') {
        const response = await fetch('http://127.0.0.1:8000/api/v1/newlogs/raw_material_status_logs/acknowledge', {
          method: 'POST',
          headers: {
        ...
        const response = await fetch('http://127.0.0.1:8000/api/v1/newlogs/machine-status-logs/acknowledge', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
