  ...
  const handleAcknowledge = async (notification) => {
    ...
    try {
      ...
      if (notification.notificationType === 'material') {
        const response = await fetch('http://172.18.7.88:7799/api/v1/newlogs/raw_material_status_logs/acknowledge', {
          method: 'POST',
          headers: {
        ...
        const response = await fetch('http://172.18.7.88:7799/api/v1/newlogs/machine-status-logs/acknowledge', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${localStorage.getItem('token')}`
