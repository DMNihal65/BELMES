  ...
  persist(
    (set) => ({
      ...
      fetchRoles: async () => {
        set({ isLoading: true });
        try {
          const response = await fetch('http://172.18.7.85:4723/api/v1/auth/roles');
      ...
      login: async (credentials) => {
        ...
        try {
          ...
          const response = await fetch('http://172.18.7.85:4723/api/v1/auth/login', {
            method: 'POST',
            headers: {
      ...
      register: async (userData) => {
        set({ isLoading: true, error: null });
        try {
          const response = await fetch('http://172.18.7.85:4723/register', {
            method: 'POST',
