  ...
  persist(
    (set) => ({
      ...
      fetchRoles: async () => {
        set({ isLoading: true });
        try {
          const response = await fetch('http://172.18.7.93:9999/api/v1/auth/roles');
      ...
      fetchMachines: async () => {
        set({ isLoading: true });
        try {
          const response = await fetch('http://172.18.7.93:9999/api/v1/master-order/all-machines/');
          const data = await response.json();
      ...
      login: async (credentials) => {
        ...
        try {
          ...
          const response = await fetch('http://172.18.7.93:9999/api/v1/auth/login', {
            method: 'POST',
            headers: {
      ...
      register: async (userData) => {
        set({ isLoading: true, error: null });
        try {
          const response = await fetch('http://172.18.7.93:9999/api/v1/auth/register', {
            method: 'POST',
