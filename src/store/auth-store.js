  ...
  persist(
    (set) => ({
      ...
      fetchRoles: async () => {
        set({ isLoading: true });
        try {
          const response = await fetch('http://172.18.100.67:4546/api/v1/auth/roles');
      ...
      login: async (credentials) => {
        ...
        try {
          ...
          if (credentials.role === 'operator') {
            ...
            response = await fetch('http://172.18.100.67:4546/api/v1/auth/machine-login', {
              method: 'POST',
              headers: {
            ...
            response = await fetch('http://172.18.100.67:4546/api/v1/auth/login', {
              method: 'POST',
              headers: {
      ...
      register: async (userData) => {
        set({ isLoading: true, error: null });
        try {
          const response = await fetch('http://172.18.100.67:4546/api/v1/auth/register', {
            method: 'POST',
      ...
      machineIdLogin: async (machineId, password) => {
        set({ isLoading: true, error: null });
        try {
          const response = await fetch('http://172.18.100.67:4546/api/v1/auth/machine-id-login', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'accept': 'application/json',
