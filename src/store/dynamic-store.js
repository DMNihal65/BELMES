...
const useDynamicStore = create((set) => ({
  ...
  fetchDynamicScheduleData: async () => {
    ...
    try {
      const response = await axios.get('http://172.18.7.155:8002/api/v1/rescheduling/reschedule-actual-planned-combined');
      set({ 
  ...
  fetchAllOrders: async () => {
    try {
      ...
      const response = await fetch('http://172.18.7.155:8002/planning/all_orders', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Accept': 'application/json'
        }
