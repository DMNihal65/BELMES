...
// const WS_MACHINES_ENDPOINT = 'ws://172.18.7.88:4545/api/v1/energymonitoring/ws/machines_data';

// Update the WebSocket endpoint for shiftwise energy data
const WS_SHIFTWISE_ENERGY_ENDPOINT = 'http://172.18.7.88:4545/api/v1/energy-monitoring/shiftwise-energy-stream';

// Add the HTTP endpoint for historical data
// const HISTORY_API_ENDPOINT = 'http://172.18.7.88:4545/api/v1/energymonitoring/shiftwise_energy_history_by_date';

// Update the endpoint constant
const MACHINE_STATUS_ENDPOINT = 'http://172.18.7.88:4545/api/v1/energy-monitoring/machine-status-stream';

// Update the endpoint constant to use the specific epoch time
const COMBINED_HISTORY_ENDPOINT = 'http://172.18.7.88:4545/api/v1/energy-monitoring/combined-history/1746586800';
...
const useEnergyMonitoringBelStore = create((set, get) => ({
  ...
  connectWebSocket: (machineId) => {
    ...
    try {
      ...
      const wsUrl = `ws://172.18.7.88:4545/api/v1/energymonitoring/ws/live_data`;
  ...
  fetchFilteredHistoryData: async (machineId, startDate, endDate, parameterName) => {
    ...
    try {
      ...
      const baseUrl = `http://172.18.7.88:4545/api/v1/energymonitoring/filtered_history_data/${machineId}?start_date=${formattedStartDate}&end_date=${formattedEndDate}&column_name=${apiParamName}`;
  ...
  fetchShiftwiseEnergyHistoryByDate: async (date) => {
    ...
    try {
      ...
      const response = await axios.get(`http://172.18.7.88:4545/api/v1/energy-monitoring/combined-history/${epochTimestamp}`, {
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
        },
  ...
  connectToParametersStream: (machineId) => {
    ...
    try {
      ...
      const eventSource = new EventSource(`http://172.18.7.88:4545/api/v1/energy-monitoring/machine/${machineId}/parameters-stream`);
  ...
  connectToParameterHistoryStream: (machineId, parameter) => {
    ...
    try {
      ...
      const eventSource = new EventSource(`http://172.18.7.88:4545/api/v1/energy-monitoring/machine/${machineId}/parameter/${parameter}/history-stream`);
  ...
  fetchParameterHistory: async (machineId, parameter, startTime, endTime) => {
    ...
    try {
      ...
      const url = `http://172.18.7.88:4545/api/v1/energy-monitoring/machine/${machineId}/parameter/${parameter}/history?start_time=${startTimestamp}&end_time=${endTimestamp}`;
