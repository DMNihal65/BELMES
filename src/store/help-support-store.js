...
const useHelpSupportStore = create((set) => ({
  ...
  fetchMachines: async () => {
    ...
    try {
      ...
      const response = await fetch(
        'http://172.18.7.88:1920/api/v1/master-order/machines/',
        {
          headers: {
  ...
  fetchMachineDocuments: async (machineId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:1920/api/v1/document-management/machine-documents/${machineId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  downloadDocument: async (minioPath, token) => {
    try {
      const response = await fetch(
        `http://172.18.7.88:1920/api/v1/document-management/documents/download/?path=${encodeURIComponent(minioPath)}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
