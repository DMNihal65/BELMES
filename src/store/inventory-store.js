...
const BASE_URL = 'http://172.18.7.93:8800/api/v1/api/inventory';
...
const addCalibration = async (calibrationData) => {
  try {
    const response = await axios.post('http://172.18.7.93:8800/api/v1/api/inventory/calibrations/', {
      ...calibrationData,
...
const updateCalibration = async (id, calibrationData) => {
  try {
    const response = await axios.put(`http://172.18.7.93:8800/api/v1/api/inventory/calibrations/${id}`, {
      ...calibrationData,
      created_by: getUserId()
    });
    return response.data;
...
const getCalibrationByItemId = async (itemId) => {
  try {
    const response = await axios.get(`http://172.18.7.93:8800/api/v1/api/inventory/calibrations/?inventory_item_id=${itemId}`);
    return response.data;
  } catch (error) {
...
const useInventoryStore = create((set, get) => ({
  ...
  fetchAllOrders: async () => {
    set({ loading: true });
    try {
      const response = await axios.get('http://172.18.7.93:8800/api/v1/planning/all_orders');
      set({ 
  ...
  submitItemRequest: async (requestData) => {
    ...
    try {
      ...
      const axiosInstance = axios.create({
        baseURL: 'http://172.18.7.93:8800/api/v1',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  fetchOperationsByPartNumber: async (partNumber) => {
    set({ loading: true });
    try {
      const response = await axios.get(`http://172.18.7.93:8800/api/v1/planning/search_order?part_number=${partNumber}`);
