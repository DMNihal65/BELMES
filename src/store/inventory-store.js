...
const useInventoryStore = create((set, get) => ({
  ...
  fetchAllOrders: async () => {
    set({ loading: true });
    try {
      const response = await axios.get('http://172.18.7.88:5654/api/v1/planning/all_orders');
      set({ 
  ...
  submitItemRequest: async (requestData) => {
    ...
    try {
      ...
      const axiosInstance = axios.create({
        baseURL: 'http://172.18.7.88:5654/api/v1',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  fetchOperationsByPartNumber: async (partNumber) => {
    set({ loading: true });
    try {
      const response = await axios.get(`http://172.18.7.88:5654/api/v1/planning/search_order?part_number=${partNumber}`);
