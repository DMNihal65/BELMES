...
const SUPERVISOR_BASE_URL = 'http://172.16.0.229:1292/api/v1/maintainance';
const OPERATOR_BASE_URL = 'http://172.16.0.229:1292/api/v1/operator';
const MASTER_ORDER_URL = 'http://172.16.0.229:1292/api/v1/master-order';
...
fetchAllMachines: async () => {
  ...
  try {
    ...
    const response = await axios.get('http://172.16.0.229:1292/api/v1/master-order/all-machines/');
...
fetchMachineCalibrationLogs: async (skip = 0) => {
  set({ loading: true, error: null });
  try {
    const response = await axios.get(`http://172.16.0.229:1292/api/v1/newlogs/machine-calibration-logs?skip=${skip}`);
    set({
