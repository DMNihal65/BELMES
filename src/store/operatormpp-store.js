...
const API_BASE_URL = 'http://172.18.7.88:5654/api/v1/planning';
const API_PLANNING_BASE_URL = 'http://172.18.7.88:5654/api/v1/planning';
...
const useOperatorMppStore = create((set, get) => ({
  ...
  fetchMachineOperations: async (machineId) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.88:5654/api/v1/operator/machines/${machineId}/operations`);
      const data = await response.json();
  ...
  activateJob: async (machineId, operationId) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch('http://172.18.7.88:5654/api/v1/logs/machine-raw-live/', {
        method: 'POST',
        headers: {
  ...
  activateOrder: async (machineId, orderData) => {
    ...
    try {
      ...
      const activateResponse = await fetch(`http://172.18.7.88:5654/api/v1/operator/machines/${machineId}/operations`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      const getResponse = await fetch(`http://172.18.7.88:5654/api/v1/operator/machines/${machineId}/operations`);
  ...
  deactivateOrder: async (machineId) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.88:5654/api/v1/operator/machines/${machineId}/operations/deactivate`, {
        method: 'POST'
      });
  ...
  submitOperatorLog: async (logData) => {
    ...
    try {
      ...
      const apiUrl = 'http://172.18.7.88:5654/api/v1/logs/operator-log';
