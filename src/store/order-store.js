...
const useOrderStore = create((set) => ({
  ...
  uploadPDF: async (file) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:7744/planning/upload-pdf', {
        method: 'POST',
        body: formData,
      });
  ...
  updateOrder: async (orderId, payload, orderNumber) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:7744/planning/update_order/${payload.orderNumber}`,
        {
          method: 'PUT',
          headers: {
  ...
  createOrder: async (payload) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:7744/planning/create_order', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  updateWorkcenter: async (workcenterData) => {
    set({ isLoadingWorkcenters: true, workcenterError: null });
    try {
      const response = await fetch(`http://172.18.7.85:7744/work_centers/${workcenterData.workcenter_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  uploadMppFile: async (file, productionOrder, documentName, description, version) => {
    try {
      ...
      const response = await fetch('http://172.18.7.85:7744/api/v1/documents/mpp/upload/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
  ...
  uploadEngineeringDrawing: async (file, productionOrder, documentName, description, version) => {
    try {
      ...
      const response = await fetch('http://172.18.7.85:7744/api/v1/documents/engineering-drawing/upload/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
        },
