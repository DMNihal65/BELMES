...
const API_CONFIG = {
  ...
  endpoints: {
    ...
    allCompletionRecords: 'http://172.16.0.203:8002/api/v1/scheduling/all-completion-records',
    orderCompletionRecord: (orderId) => `http://172.16.0.203:8002/api/v1/scheduling/order-completion-record/${orderId}`
  }
};
...
const useOrderStore = create((set, get) => ({
  ...
  updateOrder: async (updatedOrder) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.16.0.203:8002/api/v1/planning/orders/${updatedOrder.id}`,
        {
          method: 'PUT',
          headers: {
  ...
  fetchScheduledOrders: async () => {
    try {
      ...
      const response = await fetch('http://172.16.0.203:8002/api/v1/scheduling/part-production-timeline/', {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
        },
  ...
  setOrderCompletion: async (productionOrder) => {
    try {
      ...
      const url = `http://172.16.0.203:8002/api/v1/scheduling/set-order-completion/${orderId}`;
