...
const API_CONFIG = {
  BASE_URL: 'http://172.18.7.88:4493',
  QUALITY_URL: 'http://172.18.7.88:4493',
  PLANNING_URL: 'http://172.18.7.88:4493',
  SCHEDULING_URL: 'http://172.18.7.88:4493',
  endpoints: {
...
const useOrderStore = create((set, get) => ({
  ...
  uploadPDF: async (file) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.88:4493/api/v1/planning/upload-pdf', {
        method: 'POST',
        body: formData,
      });
  ...
  updateOrder: async (updatedOrder) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4493/api/v1/planning/orders/${updatedOrder.id}`,
        {
          method: 'PUT',
          headers: {
  ...
  updateWorkcenter: async (workcenterData) => {
    set({ isLoadingWorkcenters: true, workcenterError: null });
    try {
      const response = await fetch(`http://172.18.7.88:4493/api/v1/work_centers/${workcenterData.workcenter_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  swapOrderPriority: async (order1ProductionOrder, order2ProductionOrder, order1Priority, order2Priority) => {
    try {
      ...
      const priorityResponse = await fetch('http://172.18.7.88:4493/api/v1/planning/projects/priority', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        }
      });
  ...
  fetchPriorityOrders: async () => {
    ...
    try {
      const response = await fetch('http://172.18.7.88:4493/api/v1/planning/projects/priority', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        }
      });
  ...
  deleteMachine: async (machineId) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4493/api/v1/master-order/machines/${machineId}`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  checkOrderCompletion: async (partNumber, productionOrder) => {
    try {
      ...
      const url = `http://172.18.7.88:4493/api/v1/scheduling/check-order-completion-simple?part_number=${partNumber}&production_order=${productionOrder}`;
