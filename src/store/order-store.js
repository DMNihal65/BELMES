...
const useOrderStore = create((set) => ({
  ...
  uploadPDF: async (file) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:4411/planning/upload-pdf', {
        method: 'POST',
        body: formData,
      });
  ...
  updateOrder: async (orderId, payload, orderNumber) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:4411/planning/update_order/${payload.orderNumber}`,
        {
          method: 'PUT',
          headers: {
  ...
  createOrder: async (payload) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch('http://172.18.7.85:4411/create_order', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  updateWorkcenter: async (workcenterData) => {
    set({ isLoadingWorkcenters: true, workcenterError: null });
    try {
      const response = await fetch(`http://172.18.7.85:4411/work_centers/${workcenterData.workcenter_id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
