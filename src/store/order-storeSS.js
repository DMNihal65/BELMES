...
const useOrderStore = create((set, get) => ({
      ...
      try {
        ...
        if (token) {
          const documentsResponse = await fetch(
            `http://172.18.7.85:6768/api/v1/document-management/documents/by-part-number-all/${transformedData.partNumber}`,
            {
              method: 'GET',
              headers: {
  ...
  createOrder: async (orderData) => {
    try {
      ...
      const response = await fetch('http://172.18.7.85:6768/api/v1/planning/save-to-db', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  uploadMppFile: async (file, partNumber, documentName, description, version) => {
    try {
      ...
      const response = await fetch('http://172.18.7.85:6768/api/v1/document-management/documents/upload-by-type', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
