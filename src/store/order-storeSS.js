...
const useOrderStore = create((set, get) => ({
      ...
      try {
        ...
        if (token) {
          const documentsResponse = await fetch(
            `http://172.18.7.88:6999/api/v1/document-management/documents/by-part-number-all/${transformedData.partNumber}`,
            {
              method: 'GET',
              headers: {
  ...
  createOrder: async (orderData) => {
    try {
      ...
      const response = await fetch('http://172.18.7.88:6999/api/v1/planning/save-to-db', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  uploadMppFile: async (file, partNumber, documentName, description, version) => {
    try {
      ...
      const response = await fetch('http://172.18.7.88:6999/api/v1/document-management/documents/upload-by-type', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
        },
  ...
  fetchTimelineData: async () => {
    ...
    try {
      const response = await fetch('http://172.18.7.88:6999/api/v1/scheduling/part-production-timeline/');
      const data = await response.json();
  ...
  fetchDocumentsByPartNumber: async (partNumber) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:6999/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          method: 'GET',
          headers: {
            'Authorization': `Bearer ${token}`,
  ...
  swapOrderPriority: async (order1ProductionOrder, order2ProductionOrder, order1Priority, order2Priority) => {
    try {
      ...
      const allOrdersResponse = await fetch('http://172.18.7.88:6999/api/v1/planning/all_orders');
      ...
      const response = await fetch(
        `http://172.18.7.88:6999/api/v1/planning/order/${order1.id}/priority`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
