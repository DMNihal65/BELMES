...
const usePlanningStore = create((set) => ({
  ...
  checkOrderCompletion: async (partNumber, productionOrder) => {
    ...
    try {
      ...
      const response = await fetch(`http://172.18.100.67:4546/api/v1/scheduling/check-order-completion-simple/${partNumber}/${productionOrder}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json'
        },
  ...
  // Update the downloadDocument function to use direct URL with API base
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.18.100.67:4546/api/v1/scheduling/part-production-pdc11');
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/scheduling/part-production-pdc11?part_number=${partNumber}&production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
