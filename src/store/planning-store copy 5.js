    ...
    try {
      ...
      if (documentsData.mpp_document?.id) {
        ...
        const downloadResponse = await fetch(
          `http://172.18.7.89:8008/api/v1/document-management/documents/${documentsData.mpp_document.id}/download-latest`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': '*/*'
          }
...
console.log('Content-Disposition Header:', contentDisposition);
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.18.7.89:8008/api/v1/scheduling/part-production-pdc12');
  ...
  fetchEngineeringDrawings: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.89:8008/api/v1/document-management/documents/?page=1&page_size=100&part_number=${partNumber}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'accept': 'application/json'
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.89:8008/api/v1/scheduling/part-production-pdc12?part_number=${partNumber}&production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
