...
console.log('Content-Disposition Header:', contentDisposition);
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://127.0.0.1:8000/api/v1/scheduling/part-production-pdc12');
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/scheduling/part-production-pdc12?part_number=${partNumber}&production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
