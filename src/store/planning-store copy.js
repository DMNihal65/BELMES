...
const usePlanningStore = create((set) => ({
  ...
  searchOrders: async (partNumber) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.85:6639/planning/search_order2?part_number=${partNumber}`);
      const data = await response.json();
  ...
  fetchMPPDetails: async (partNumber, operationNumber) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:6639/mpp/by-identifier?operation_number=${operationNumber}&part_number=${partNumber}`
      );
  ...
  saveMPPDetails: async (mppData) => {
    ...
    try {
      ...
      const formattedData = {
        production_order: String(mppData.production_order).trim(),
        ...
        work_instructions: Array.isArray(mppData.work_instructions) 
          ? mppData.work_instructions.map((instruction, index) => ({
              title: String(instruction.title || '').trim(),
              instructions: String(instruction.instructions || '').trim(),
              sequence: index + 1
            }))
          : []
      };

      // Remove part_number from the request
      // const { part_number, ...requestData } = formattedData;

      console.log('Sending MPP data:', formattedData);

      const response = await fetch('http://172.18.7.85:6639/mpp', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.detail || `Failed to save MPP details (${response.status})`);
      }
  ...
  fetchActiveParts: async () => {
    try {
      const response = await fetch('http://172.18.7.85:6639/scheduling/active-parts');
      const data = await response.json();
  ...
  changePartStatus: async (partNumber, newStatus) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:6639/scheduling/set-part-status/${partNumber}?status=${newStatus}`, {
  ...
  fetchMachineDetails: async (machineId) => {
    try {
      const response = await fetch(`http://172.18.7.85:6639/master-order/machines/${machineId}`);
      const data = await response.json();
  ...
  //   try {
  //     const response = await fetch(`http://172.18.7.85:6639/planning/operations/${partNumber}/${operationNumber}`, {
  //       method: 'PUT',
  //       headers: {
  //         'Content-Type': 'application/json',
  //       },
