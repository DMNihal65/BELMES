...
const usePlanningStore = create((set) => ({
  ...
  updateOperationDetails: async (partNumber, operationNumber, updateData) => {
    try {
      const response = await fetch(`http://172.18.7.91:8008/api/v1/planning/operations/${partNumber}/${operationNumber}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    try {
      ...
      if (documentsData.mpp_document?.latest_version?.id) {
        ...
        const downloadResponse = await fetch(
          `http://172.18.7.91:8008/api/v1/document-management/documents/${documentsData.mpp_document.latest_version.id}/download-latest`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': '*/*'
          }
    ...
    try {
      ...
      if (workCenterMachines && workCenterMachines.length > 0) {
        ...
        try {
          ...
          await fetch(`http://172.18.7.91:8008/api/v1/planning/operations/${partNumber}/${operationData.operation_number}`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`,
              'Content-Type': 'application/json',
  ...
  fetchMachineUtilization: async (month, year) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/scheduling/machine-utilization?month=${month}&year=${year}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  fetchMachineUtilizationByDateRange: async (startDate, endDate) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/scheduling/machine-utilization/range?start_date=${formattedStartDate}&end_date=${formattedEndDate}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  updateOrderTool: async (toolId, toolData) => {
    try {
      ...
      const apiUrl = `http://172.18.7.91:8008/api/v1/toolsprograms/ordertools/${toolId}/`;
  ...
  addOrderProgram: async (programData) => {
    try {
      ...
      const response = await fetch('http://172.18.7.91:8008/api/v1/toolsprograms/programs/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  fetchProgramsByOrderId: async (orderId) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/toolsprograms/programs/?order_id=${orderId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  updateOrderProgram: async (programId, programData) => {
    try {
      ...
      const apiUrl = `http://172.18.7.91:8008/api/v1/toolsprograms/programs/${programId}/`;
      console.log(`Sending PUT request to: ${apiUrl}`);
  ...
  deleteOrderProgram: async (programId) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/toolsprograms/programs/${programId}/`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
  ...
  updateProgramVersion: async (programId, file, versionNumber) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/document-management/cnc-program/${programId}/versions`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  fetchProgramVersions: async (documentId) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/document-management/cnc-program/${documentId}/versions`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.18.7.91:8008/api/v1/scheduling/part-production-pdc');
  ...
  fetchEngineeringDrawings: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.91:8008/api/v1/document-management/documents/?page=1&page_size=10&part_number=${partNumber}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'accept': 'application/json'
    ...
    try {
      ...
      if (!documentName) {
        try {
          ...
          const docDetailsResponse = await fetch(`http://172.18.7.91:8008/api/v1/document-management/documents/${documentId}`, {
            headers: {
              'Authorization': `Bearer ${token}`,
              'accept': 'application/json'
            }
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.91:8008/api/v1/scheduling/part-production-pdc`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  fetchPdcData: async (productionOrder) => {
    try {
      const response = await fetch(`http://172.18.7.91:8008/api/v1/scheduling/part-production-pdc?production_order=${productionOrder}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
