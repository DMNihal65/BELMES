...
const usePlanningStore = create((set) => ({
  ...
  searchOrders: async (partNumber) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.85:4412/planning/search_order?part_number=${partNumber}`);
      const data = await response.json();
  ...
  fetchMPPDetails: async (partNumber, operationNumber) => {
    ...
    try {
      const response = await fetch(`http://172.18.7.85:4412/mpp/by-part/${partNumber}/${operationNumber}`);
  ...
  saveMPPDetails: async (mppData) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:4412/mpp', {
        method: 'POST',
        headers: {
  ...
  fetchActiveParts: async () => {
    try {
      const response = await fetch('http://172.18.7.85:4412/scheduling/active-parts');
      const data = await response.json();
  ...
  changePartStatus: async (partNumber, newStatus) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:4412/scheduling/set-part-status/${partNumber}?status=${newStatus}`, {
