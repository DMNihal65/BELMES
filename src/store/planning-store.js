    ...
    try {
      ...
      if (documentsData.mpp_document?.id) {
        ...
        const downloadResponse = await fetch(
          `http://172.18.7.88:4493/api/v1/document-management/documents/${documentsData.mpp_document.id}/download-latest`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': '*/*'
          }
  ...
  fetchEngineeringDrawings: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:4493/api/v1/document-management/documents/?page=1&page_size=100&part_number=${partNumber}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'accept': 'application/json'
