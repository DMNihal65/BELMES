...
const usePlanningStore = create((set) => ({
  ...
  changePartStatus: async (partNumber, newStatus) => {
    try {
      ...
      const response = await fetch(`http://172.16.0.203:8002/api/v1/scheduling/set-part-status/${partNumber}?status=${newStatus}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  checkIpidStatus: async (productionOrder, operationNumber) => {
    try {
      // Update the endpoint URL from http://172.16.0.203:8002 to http://172.16.0.203:8002
      const response = await fetch(`http://172.16.0.203:8002/api/v1/document-management/ipid/status?production_order=${productionOrder}&operation_number=${operationNumber}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.16.0.203:8002/api/v1/rescheduling/part-production-pdc');
  ...
  fetchEngineeringDrawings: async (partNumber = null, pageNumber = 1, pageSize = 10) => {
    try {
      ...
      let url = `http://172.16.0.203:8002/api/v1/document-management/documents/?page=${pageNumber}&page_size=${pageSize}`;
