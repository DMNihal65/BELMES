...
const usePlanningStore = create((set) => ({
  ...
  changePartStatus: async (partNumber, newStatus) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:8078/api/v1/scheduling/set-part-status/${partNumber}?status=${newStatus}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  checkIpidStatus: async (productionOrder, operationNumber) => {
    try {
      // Update the endpoint URL from http://172.18.7.93:9999 to http://172.18.7.85:8078
