...
const usePlanningStore = create((set) => ({
  ...
  searchOrders: async (partNumber) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.85:7739/planning/search_order2?part_number=${partNumber}`);
      const data = await response.json();
  ...
  fetchMPPDetails: async (partNumber, operationNumber) => {
    ...
    try {
      const response = await fetch(`http://172.18.7.85:7739/mpp/by-part/${partNumber}/${operationNumber}`);
