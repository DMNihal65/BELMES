...
const usePlanningStore = create((set) => ({
  ...
  fetchMachinePlanningByDateRange: async (startDate, endDate) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.93:8800/api/v1/scheduling/machine-utilization/range?start_date=${formattedStartDate}&end_date=${formattedEndDate}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.93:8800/api/v1/scheduling/part-production-pdc?production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
  ...
  fetchProgramsByOrderId: async (orderId) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.93:8800/api/v1/toolsprograms/orderprograms/by-order/${orderId}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'accept': 'application/json'
        }
  ...
  addOrderProgram: async (programData) => {
    try {
      ...
      const response = await fetch('http://172.18.7.93:8800/api/v1/toolsprograms/orderprograms/', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
  ...
  updateOrderProgram: async (programId, programData) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.93:8800/api/v1/toolsprograms/orderprograms/${programId}`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json',
