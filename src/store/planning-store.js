...
const usePlanningStore = create((set) => ({
  ...
  fetchAllOrders: async () => {
    ...
    try {
      const response = await fetch('http://172.18.7.85:6797/api/v1/planning/all_orders');
  ...
  searchOrders: async (productionOrder) => {
    set({ isLoading: true, error: null });
    try {
      const response = await fetch(`http://172.18.7.85:6797/api/v1/planning/search_order2?production_order=${productionOrder}`);
      const data = await response.json();
  ...
  fetchMPPDetails: async (productionOrder) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:6797/api/v1/documents/mpp/${selectedOrder}`, {
        headers: {
  ...
  saveMPPDetails: async (mppData) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:6797/api/v1/mpp', {
        method: 'POST',
        headers: {
  ...
  fetchActiveParts: async () => {
    try {
      const response = await fetch('http://172.18.7.85:6797/api/v1/scheduling/active-parts');
      const data = await response.json();
  ...
  changePartStatus: async (partNumber, newStatus) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:6797/api/v1/scheduling/set-part-status/${partNumber}?status=${newStatus}`, {
  ...
  fetchMachineDetails: async (machineId) => {
    try {
      const response = await fetch(`http://172.18.7.85:6797/api/v1/master-order/machines/${machineId}`);
      const data = await response.json();
  ...
  updateOperationDetails: async (partNumber, operationNumber, updateData) => {
    try {
      const response = await fetch(`http://172.18.7.85:6797/api/v1/planning/operations/${partNumber}/${operationNumber}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  fetchMppDocuments: async (productionOrder) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:6797/api/v1/documents/mpp/${productionOrder}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'accept': 'application/json'
        }
  ...
  fetchMppByIdentifier: async (productionOrder, operationNumber) => {
    try {
      const response = await fetch(`http://172.18.7.85:6797/api/v1/mpp/by-identifier?operation_number=${operationNumber}&production_order=${productionOrder}`, {
        headers: {
          'accept': 'application/json'
        }
      });
  ...
  downloadMppDocument: async (versionId) => {
    try {
      ...
      const response = await fetch(`http://172.18.7.85:6797/api/v1/documents/${versionId}/download`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'accept': 'application/json'
        }
