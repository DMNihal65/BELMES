...
const usePlanningStore = create((set) => ({
  ...
  checkOrderCompletion: async (partNumber, productionOrder) => {
    ...
    try {
      ...
      const response = await fetch(`http://172.16.0.229:1292/api/v1/scheduling/check-order-completion/${partNumber}/${productionOrder}`, {
        method: 'GET',
        headers: {
          'Accept': 'application/json'
        },
...
console.log('Content-Disposition Header:', contentDisposition);
  ...
  fetchPartProductionPDC: async (partNumber, productionOrder) => {
    try {
      ...
      const url = new URL('http://172.16.0.229:1292/api/v1/scheduling/pdc/by-part-po');
  ...
  fetchPdcForCurrentJob: async (productionOrder) => {
    try {
      ...
      const response = await fetch(
        `http://172.16.0.229:1292/api/v1/scheduling/pdc/by-part-po?part_number=${partNumber}&production_order=${productionOrder}`, 
        {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'accept': 'application/json'
