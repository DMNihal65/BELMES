...
class QualityStore {
  ...
  async fetchBalloonedDrawing(drawingId, operationId) {
    try {
      const response = await axios.get(
        `http://172.18.7.93:8800/api/v1/document-management/ballooned-drawing/download/${drawingId}/${operationId}`,
        {
          ...this.getAuthHeaders(),
  ...
  async fetchReportStructure(forceRefresh = false) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.93:8800/api/v1/document-management/report/structure/?force_refresh=${forceRefresh}`,
        this.getAuthHeaders()
      );
