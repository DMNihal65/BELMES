...
class QualityStore {
  ...
  async fetchAllOrders() {
    try {
      const response = await axios.get(
        'http://172.18.7.85:7708/api/v1/planning/all_orders',
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionByOrderId(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:7708/quality/inspection/${orderId}/detailed`,
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionDetails(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:7708/quality/master-boc/ipids/${orderId}`,
        this.getAuthHeaders()
      );
  ...
  async launchQMSSoftware() {
    try {
      const response = await axios.get(
        'http://172.18.7.85:7708/api/v1/quality/run',
        this.getAuthHeaders()
      );
