...
class QualityStore {
  ...
  async fetchInspectionByOrderId(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.155:8002/quality/inspection/${orderId}/detailed`,
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionDetails(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.155:8002/quality/master-boc/ipids/${orderId}`,
        this.getAuthHeaders()
      );
  ...
  async launchQMSSoftware() {
    try {
      const response = await axios.get(
        'http://172.18.7.155:8002/api/v1/quality/run',
        this.getAuthHeaders()
      );
