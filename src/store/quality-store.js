...
class QualityStore {
  ...
  async fetchAllOrders() {
    try {
      const response = await axios.get(
        'http://172.18.7.88:9422/api/v1/planning/all_orders',
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionByOrderId(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.88:9422/api/v1/quality/inspection/${orderId}/detailed`,
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionDetails(orderId) {
    try {
      ...
      const config = {
        method: 'get',
        url: `http://172.18.7.88:9422/api/v1/quality/master-boc/ipids/${orderId}`,
        ...this.getAuthHeaders()
      };
  ...
  async launchQMSSoftware() {
    try {
      const response = await axios.get(
        'http://172.18.7.88:9422/api/v1/quality/run',
        this.getAuthHeaders()
      );
      return response.data;
    } catch (error) {
  ...
  async fetchBalloonedDrawing(drawingId, operationId) {
    try {
      const response = await axios.get(
        `http://172.18.7.88:9422/api/v1/document-management/ballooned-drawing/download/${drawingId}/${operationId}`,
        {
          ...this.getAuthHeaders(),
      ...
      for (const method of methods) {
        try {
          ...
          const response = await axios({
            method: method,
            url: `http://172.18.7.88:9422/api/v1/quality/stage-inspection/${inspectionId}/status?is_done=${isDone}`,
            ...this.getAuthHeaders(),
            timeout: 5000
          });
  ...
  async checkNetworkConnectivity() {
    try {
      ...
      await axios({
        method: 'head',
        url: 'http://172.18.7.88:9422/api/v1/health', // Use a health endpoint if available
  ...
  async fetchReportStructure(forceRefresh = false) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.88:9422/api/v1/document-management/report/structure/?force_refresh=${forceRefresh}`,
        this.getAuthHeaders()
      );
  ...
  async downloadReport(filePath) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.88:9422/api/v1/document-management/download/?path=${encodeURIComponent(filePath)}`,
        {
          ...this.getAuthHeaders(),
  ...
  async fetchDetailedInspection(inspectionId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.88:9422/api/v1/quality/inspection/${inspectionId}/detailed`,
        this.getAuthHeaders()
      );
