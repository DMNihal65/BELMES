...
class QualityStore {
  ...
  async fetchAllOrders() {
    try {
      const response = await axios.get(
        'http://172.18.7.85:2078/api/v1/planning/all_orders',
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionByOrderId(orderId) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:2078/api/v1/quality/inspection/${orderId}/detailed`,
        this.getAuthHeaders()
      );
  ...
  async fetchInspectionDetails(orderId) {
    try {
      ...
      const config = {
        method: 'get',
        url: `http://172.18.7.85:2078/api/v1/quality/master-boc/ipids/${orderId}`,
        ...this.getAuthHeaders()
      };
  ...
  async launchQMSSoftware() {
    try {
      const response = await axios.get(
        'http://172.18.7.85:2078/api/v1/quality/run',
        this.getAuthHeaders()
      );
      return response.data;
    } catch (error) {
  ...
  async fetchBalloonedDrawing(drawingId, operationId) {
    try {
      const response = await axios.get(
        `http://172.18.7.85:2078/api/v1/document-management/ballooned-drawing/download/${drawingId}/${operationId}`,
        {
          ...this.getAuthHeaders(),
  ...
  async updateInspectionStatus(inspectionId, isDone) {
    try {
      ...
      const endpointPatterns = [
        ...
        {
          method: 'put',
          url: `http://172.18.7.85:2078/api/v1/quality/stage-inspection/${inspectionId}/status?is_done=${isDone}`,
          data: {}
        },
        // Alternative with ID in path parameter (PATCH)
        {
          method: 'patch',
          url: `http://172.18.7.85:2078/api/v1/quality/stage-inspection/${inspectionId}/status`,
          data: { is_done: isDone }
        },
        // Alternative with ID in path and status in path (PUT)
        {
          method: 'put',
          url: `http://172.18.7.85:2078/api/v1/quality/stage-inspection/${inspectionId}/${isDone ? 'complete' : 'incomplete'}`,
          data: {}
        },
        // Alternative with different base path (PUT)
        {
          method: 'put',
          url: `http://172.18.7.85:2078/api/v1/quality/inspection/${inspectionId}/status?is_done=${isDone}`,
          data: {}
        },
        // Alternative with different base path (POST)
        {
          method: 'post',
          url: `http://172.18.7.85:2078/api/v1/quality/inspection/${inspectionId}/status`,
          data: { is_done: isDone }
        },
        // Additional alternative with direct update to inspection record
        {
          method: 'put',
          url: `http://172.18.7.85:2078/api/v1/quality/inspections/${inspectionId}`,
          data: { is_done: isDone }
        },
  ...
  async checkNetworkConnectivity() {
    try {
      ...
      await axios({
        method: 'head',
        url: 'http://172.18.7.85:2078/api/v1/health', // Use a health endpoint if available
  ...
  async fetchReportStructure(forceRefresh = false) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:2078/api/v1/document-management/report/structure/?force_refresh=${forceRefresh}`,
        this.getAuthHeaders()
      );
  ...
  async downloadReport(filePath) {
    try {
      ...
      const response = await axios.get(
        `http://172.18.7.85:2078/api/v1/document-management/download/?path=${encodeURIComponent(filePath)}`,
        {
          ...this.getAuthHeaders(),
