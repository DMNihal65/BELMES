...
const useScheduleStore = create((set, get) => ({
  ...
  fetchScheduleData: async () => {
    ...
    try {
      const response = await fetch('http://172.18.7.88:4545/operations/schedule-batch/');
...
//check 
  ...
  ////unit schedule
    ...
    //   set({ loading: true, error: null });
    //   try {
    //     const response = await fetch('http://172.18.7.88:4545/operations/unit_schedule/');
  ...
  fetchProductionStatus: async (partNumber, startEpoch, endEpoch) => {
    ...
    try {
      let url = `http://172.18.7.88:4545/production/daily/?start_epoch=${startEpoch}&end_epoch=${endEpoch}`;
      if (partNumber) {
  ...
  fetchWeeklyProductionStatus: async (partNumber, startEpoch, endEpoch) => {
    set({ productionStatusLoading: true, productionStatusError: null });
    try {
      let url = `http://172.18.7.88:4545/production/weekly/?start_epoch=${startEpoch}&end_epoch=${endEpoch}`;
      if (partNumber) {
        url += `&part_number=${partNumber}`;
      }
      const response = await axios.get(url);
  ...
  fetchMonthlyProductionStatus: async (partNumber, startEpoch, endEpoch) => {
    set({ productionStatusLoading: true, productionStatusError: null });
    try {
      let url = `http://172.18.7.88:4545/production/monthly/?start_epoch=${startEpoch}&end_epoch=${endEpoch}`;
      if (partNumber) {
        url += `&part_number=${partNumber}`;
      }
      const response = await axios.get(url);
  ...
  fetchPartNumberSuggestions: async (searchText) => {
    try {
      const response = await axios.get(`http://172.18.7.88:4545/production/daily/?start_epoch=1739245961&end_epoch=1739245961`);
