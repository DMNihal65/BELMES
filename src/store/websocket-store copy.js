...
const useWebSocketStore = create((set, get) => ({
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3502/api/v1/maintainance/operator/machine-update/${machineId}`, // Moved URL here
        {
          method: 'POST',
          headers: {
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3502/api/v1/maintainance/operator/raw-material-update/${partNumber}`, // Moved URL here
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
