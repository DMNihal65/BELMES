...
const useWebSocketStore = create((set, get) => ({
  ...
  initializeWebSocket: (machineId) => {
    ...
    const ws = new WebSocket('ws://172.18.7.88:3252/production_monitoring/ws/live-status/');
  ...
  fetchMachineOperations: async (machineId) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/operator/machines/${machineId}/operations`
      );
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/maintainance/operator/machine-update/${machineId}`, // Moved URL here
        {
          method: 'POST',
          headers: {
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/maintainance/operator/raw-material-update/${partNumber}`, // Moved URL here
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  submitBreakdownIssue: async (values) => {
    ...
    try {
      const response = await fetch('http://172.18.7.88:3252/api/v1/maintainance/downtimes/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  fetchDocuments: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          headers: {
  ...
  downloadDocument: async (partNumber, docType) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/document-management/documents/download-latest/${partNumber}/${docType}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  fetchMppDetails: async (partNumber, operationNumber) => {
    try {
      const response = await fetch(
        `http://172.18.7.88:3252/api/v1/mpp/by-part/${partNumber}/${operationNumber}`
      );
