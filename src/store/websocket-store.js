...
const useWebSocketStore = create((set, get) => ({
  ...
  initializeWebSocket: (machineId) => {
    ...
    const ws = new WebSocket('ws://172.18.7.85:9738/production_monitoring/ws/live-status/');
  ...
  fetchMachineOperations: async (machineId) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:9738/api/v1/operator/machines/${machineId}/operations`
      );
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:9738/api/v1/maintainance/operator/machine-update/${machineId}`,
        {
          method: 'POST',
          headers: {
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:9738/api/v1/maintainance/operator/raw-material-update/${partNumber}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  fetchDocuments: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:9738/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          headers: {
  ...
  downloadDocument: async (partNumber, docType) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.85:9738/api/v1/document-management/documents/download-latest/${partNumber}/${docType}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
