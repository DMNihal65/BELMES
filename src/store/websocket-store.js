...
const useWorkcenterStore = create((set, get) => ({
  ...
  updateWorkcenter: async (updatedItem) => {
    ...
    try {
      ...
      const response = await fetch(`http://172.18.7.88:3252/api/v1/master-order/machines/${updatedItem.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  createWorkcenter: async (workcenterData) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.88:3252/api/v1/master-order/workcenters/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
      ...
      const [workcentersResponse, allMachinesResponse] = await Promise.all([
        fetch('http://172.18.7.88:3252/api/v1/master-order/workcenters/?skip=0&limit=100'),
        fetch('http://172.18.7.88:3252/api/v1/master-order/all-machines/')
      ]);
