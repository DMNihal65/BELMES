...
const useWebSocketStore = create((set, get) => ({
  ...
  fetchOperationDetails: async (machineId, operationNumber, partNumber) => {
    try {
      ...
      const [operationsResponse, documentsResponse] = await Promise.all([
        fetch(`http://172.18.7.85:8078/api/v1/operator/machines/${machineId}/operations`),
        fetch(
          `http://172.18.7.85:8078/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
          {
            headers: {
  ...
  downloadDocumentById: async (documentId) => {
    try {
      ...
      const endpoint = `http://172.18.7.85:8078/api/v1/document-management/documents/${documentId}/download-latest`;
