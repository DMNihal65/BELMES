...
const useWebSocketStore = create((set, get) => ({
  ...
  initializeWebSocket: (machineId) => {
    ...
    const ws = new WebSocket('ws://172.18.100.67:4546/production_monitoring/ws/live-status/');
  ...
  fetchMachineOperations: async (machineId, persistenceOverride = false) => {
    try {
      ...
      if (!persistenceOverride && activeJobSource === 'user-selected' && storedCurrentJobData && storedJobData) {
        try {
          ...
          if (storedActiveOperation) {
            userSelectedOperation = JSON.parse(storedActiveOperation);
          }
          
          console.log('Using user-selected job data:', userSelectedJob);
        } catch (parseError) {
          console.error('Error parsing stored user-selected job:', parseError);
        }
      }

      console.log(`Fetching machine operations for machine ID: ${machineId}`);
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/operator/machines/${machineId}/operations`
      );
      ...
      // IMPORTANT: Check if there's an active job from the API
      const hasApiActiveJob = formattedOperations.inprogress && formattedOperations.inprogress.length > 0;
      
      if (hasApiActiveJob) {
        // API says there's an active job, use that data
        if (data.orders && data.orders.length > 0) {
          currentJob = createJobDataFromOrderResponse(data, machineId, formattedOperations);
          
          // Mark this as API-provided job data
      ...
      // only if persistenceOverride is false
        ...
        if (userSelectedOperation) {
          ...
          const updatedOperations = {
            ...formattedOperations,
            inprogress: [userSelectedOperation],
            // Remove the operation from scheduled if it's there
            scheduled: formattedOperations.scheduled.filter(
              op => op.id !== userSelectedOperation.id
            )
          };
          
          // Update operations with our user selection
          localStorage.setItem('operationsData', JSON.stringify(updatedOperations));
          
          // Create current job data from user selection
          currentJob = createJobDataFromUserSelection(
            userSelectedJob, 
            machineId, 
            updatedOperations,
          ...
          // No operation selected, just use the job data
          currentJob = createJobDataFromUserSelection(
            userSelectedJob,
            machineId,
            formattedOperations
      ...
      // If we have a job, save it
      if (currentJob) {
        // Store job data
        localStorage.setItem('currentJobData', JSON.stringify(
          currentJob.project ? currentJob : userSelectedJob
        ));
        ...
        if (formattedOperations.inprogress.length > 0) {
          ...
          localStorage.setItem('activeOperation', JSON.stringify(userSelectedOperation));
        }
      } else {
  ...
  // Helper function to create job data from order response
  createJobDataFromOrderResponse: (data, machineId, formattedOperations) => {
  ...
  // Helper function to create job data from user selection
  createJobDataFromUserSelection: (userSelectedJob, machineId, formattedOperations, userSelectedOperation = null) => {
    // Get machine status from localStorage
    const storedMachineStatus = localStorage.getItem('machineStatus');
    const machineStatus = storedMachineStatus ? JSON.parse(storedMachineStatus) : null;
    ...
    return {
      ...
      machine: {
        ...(existingJobData?.machine || {}),
        id: parseInt(machineId),
        name: existingJobData?.machine?.name || 'Unknown Machine',
        status: machineStatus?.status || existingJobData?.machine?.status || 'IDLE',
        efficiency: existingJobData?.machine?.efficiency || 92,
        currentCycle: existingJobData?.machine?.currentCycle || '02:45',
        nextMaintenance: existingJobData?.machine?.nextMaintenance || '4hrs',
        alerts: existingJobData?.machine?.alerts || 0,
        totalParts: userSelectedJob.required_quantity || userSelectedJob.required_qty || 0,
        completedParts: machineStatus?.part_count || existingJobData?.machine?.completedParts || 0
      },
      currentOperation: userSelectedOperation || 
                        (formattedOperations.inprogress.length > 0 ? 
                         formattedOperations.inprogress[0] : null),
      operations: [...formattedOperations.completed, ...formattedOperations.inprogress, ...formattedOperations.scheduled]
    };
  },

  submitBreakdownIssue: async (payload) => {
    try {
      ...
      if (typeof payload.reported_by === 'undefined' || payload.reported_by === null) {
        throw new Error('Reported by user ID is required');
      }

      const response = await fetch('http://172.18.100.67:4546/api/v1/maintainance/downtimes/', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || 'Failed to submit breakdown issue');
      }
      ...
      return { success: false, error: error.message || 'Failed to submit breakdown issue' };
    } finally {
      set({ maintenanceLoading: false });
    }
  },

  submitOeeIssueReport: async (payload) => {
    try {
      set({ maintenanceLoading: true });
      ...
      const response = await fetch('http://172.18.100.67:4546/api/v1/operator/issues/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.detail || 'Failed to submit OEE Issue report');
      }

      const data = await response.json();
      return { success: true, data };
    } catch (error) {
      console.error('Error submitting OEE Issue report:', error);
      return { success: false, error: error.message || 'Failed to submit OEE Issue report' };
    } finally {
      set({ maintenanceLoading: false });
    }
  },
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/maintainance/operator/machine-update/${machineId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/maintainance/operator/raw-material-update/${partNumber}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  fetchDocuments: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          headers: {
  ...
  downloadDocument: async (partNumber, docType) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/document-management/documents/download-latest/${partNumber}/${docType}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  fetchMppDetails: async (partNumber, operationNumber) => {
    try {
      const response = await fetch(
        `http://172.18.100.67:4546/api/v1/mpp/by-part/${partNumber}/${operationNumber}`
      );
  ...
  fetchOperationDetails: async (machineId, operationNumber, partNumber) => {
    try {
      ...
      const [operationsResponse, documentsResponse] = await Promise.all([
        fetch(`http://172.18.100.67:4546/api/v1/operator/machines/${machineId}/operations`),
        fetch(
          `http://172.18.100.67:4546/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
          {
            headers: {
  ...
  downloadDocumentById: async (documentId) => {
    try {
      ...
      const endpoint = `http://172.18.100.67:4546/api/v1/document-management/documents/${documentId}/download-latest`;
