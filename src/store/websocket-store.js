...
const useWebSocketStore = create((set, get) => ({
  ...
  initializeWebSocket: (machineId) => {
    ...
    const ws = new WebSocket('ws://172.18.7.88:5458/production_monitoring/ws/live-status/');
  ...
  fetchMachineOperations: async (machineId, persistenceOverride = false) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/operator/machines/${machineId}/operations`
      );
  ...
  submitBreakdownIssue: async (values) => {
    ...
    try {
      const response = await fetch('http://172.18.7.88:5458/api/v1/maintainance/downtimes/', {
        method: 'POST',
        headers: {
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/maintainance/operator/machine-update/${machineId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/maintainance/operator/raw-material-update/${partNumber}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  fetchDocuments: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          headers: {
  ...
  downloadDocument: async (partNumber, docType) => {
    try {
      ...
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/document-management/documents/download-latest/${partNumber}/${docType}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  fetchMppDetails: async (partNumber, operationNumber) => {
    try {
      const response = await fetch(
        `http://172.18.7.88:5458/api/v1/mpp/by-part/${partNumber}/${operationNumber}`
      );
  ...
  fetchOperationDetails: async (machineId, operationNumber, partNumber) => {
    try {
      ...
      const [operationsResponse, documentsResponse] = await Promise.all([
        fetch(`http://172.18.7.88:5458/api/v1/operator/machines/${machineId}/operations`),
        fetch(
          `http://172.18.7.88:5458/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
          {
            headers: {
  ...
  downloadDocumentById: async (documentId) => {
    try {
      ...
      const endpoint = `http://172.18.7.88:5458/api/v1/document-management/documents/${documentId}/download-latest`;
