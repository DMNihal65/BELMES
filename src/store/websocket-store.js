...
const useWebSocketStore = create((set, get) => ({
  ...
  initializeWebSocket: (machineId) => {
    ...
    const ws = new WebSocket('ws://127.0.0.1:8000/production_monitoring/ws/live-status/');
  ...
  fetchMachineOperations: async (machineId, persistenceOverride = false) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/operator/machines/${machineId}/operations`
      );
  ...
  submitBreakdownIssue: async (payload) => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/v1/maintainance/downtimes/', {
        method: 'POST',
        headers: {
  ...
  submitOeeIssueReport: async (payload) => {
    try {
      ...
      const response = await fetch('http://127.0.0.1:8000/api/v1/operator/issues/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  submitMachineIssue: async (machineId, payload) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/maintainance/operator/machine-update/${machineId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  submitComponentIssue: async (partNumber, payload) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/maintainance/operator/raw-material-update/${partNumber}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  fetchDocuments: async (partNumber) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
        {
          headers: {
  ...
  downloadDocument: async (partNumber, docType) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/document-management/documents/download-latest/${partNumber}/${docType}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Accept': 'application/json'
  ...
  fetchMppDetails: async (partNumber, operationNumber) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/mpp/by-part/${partNumber}/${operationNumber}`
      );
  ...
  fetchOperationDetails: async (machineId, operationNumber, partNumber) => {
    try {
      ...
      const [operationsResponse, documentsResponse] = await Promise.all([
        fetch(`http://127.0.0.1:8000/api/v1/operator/machines/${machineId}/operations`),
        fetch(
          `http://127.0.0.1:8000/api/v1/document-management/documents/by-part-number-all/${partNumber}`,
          {
            headers: {
  ...
  downloadDocumentById: async (documentId) => {
    try {
      ...
      const endpoint = `http://127.0.0.1:8000/api/v1/document-management/documents/${documentId}/download-latest`;
