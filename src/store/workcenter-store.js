...
export const fetchAllMachines = async () => {
  try {
    const response = await fetch('http://172.18.7.85:4413/master-order/machines/');
    if (!response.ok) {
...
const useWorkcenterStore = create((set, get) => ({
  ...
  fetchWorkcenters: async () => {
    ...
    try {
      const response = await fetch('http://172.18.7.85:4413/master-order/all-machines/');
      if (!response.ok) {
  ...
  updateWorkcenter: async (updatedItem) => {
    ...
    try {
      ...
      const response = await fetch(`http://172.18.7.85:4413/master-order/machines/${updatedItem.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  createWorkcenter: async (workcenterData) => {
    ...
    try {
      ...
      const response = await fetch('http://172.18.7.85:4413/master-order/workcenters/', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Accept': 'application/json'
      ...
      const [workcentersResponse, allMachinesResponse] = await Promise.all([
        fetch('http://172.18.7.85:4413/master-order/workcenters/?skip=0&limit=100'),
        fetch('http://172.18.7.85:4413/master-order/all-machines/')
      ]);
