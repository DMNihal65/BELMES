...
const useGanttStore = create((set, get) => ({
  ...
  fetchGanttData: async (forceRefresh = false) => {
    ...
    try {
      ...
      // Only add date parameters if not getting all data
      if (!forceRefresh && dateRange?.[0] && dateRange?.[1]) {
        ...
        console.log('Using date range:', { formattedStartDate, formattedEndDate });
      }
